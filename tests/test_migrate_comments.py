...
def test_existing_library_gains_comments_table_without_losing_rows(tmp_path: Path) -> None:
    ...
    conn.executescript(
        ...
        INSERT INTO schema_migrations(id, applied_at) VALUES
            ('001_init', '2026-01-01T00:00:00+00:00'),
            ('002_reading_status', '2026-01-02T00:00:00+00:00');
    ...
    assert "paper_comments" in tables
    paper = conn.execute("SELECT title FROM papers WHERE zotero_item_key = 'OLDKEY01'").fetchone()
    assert paper["title"] == "Legacy paper"
    assert conn.execute("SELECT id FROM schema_migrations WHERE id = '003_paper_comments'").fetchone()
    conn.close()
